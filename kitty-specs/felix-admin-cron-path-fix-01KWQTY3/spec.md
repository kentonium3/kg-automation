...
| FR-003 | The now-**false** cwd *warning prose* that FR-001 obsoletes MUST be corrected/removed — the "cwd matters / running from elsewhere produces ModuleNotFoundError" warning in the habits prompt and the "Working dir: /home/claude/kg-automation" line in the capture prompt. The functional `cd /home/claude/kg-automation && …` prefixes are **kept** as harmless redundancy (removing a working safety net is deferred to #658, sequenced after the guardrail is deployed AND verified live per SC-10 — guardrail-first). | Draft |
...
| FR-008 | **(NARROWED — see #659)** The inbox agent's historical forensic logs (`inbox-prescan-*.md`) under `/home/claude/second-brain/agents/logs/` MUST be preserved into the canonical vault. The migration MUST NOT remove the `/home/claude/second-brain` tree itself: the tree still hosts an **active** subsystem (observation-digest / felix-core-digest via `scripts/openclaw/observation/config.py`) that is out of this mission's scope. The **full decommission** of `/home/claude/second-brain` (repointing the observation-digest writers, then removing the tree) is tracked as the fast-follow **#659**. Any state/log copy MUST still be non-destructive (no data loss). | Draft |
...
- **SC-5 (NARROWED — see #659)**: The **inbox** writers no longer target `/home/claude/second-brain` (achieved by WP02/03/04) and the inbox state + historical inbox forensic logs are migrated to their canonical homes. The stray tree itself is intentionally **left in place** this mission because the active observation-digest subsystem still writes there; its full decommission is the fast-follow **#659**.
...
- Repointing the **observation-digest** logging (`scripts/openclaw/observation/config.py` → `Path.home()/second-brain`, `log_action.py`, `summarize.py`, felix-core-digest / F014) and the **full decommission** of `/home/claude/second-brain` — deferred to the fast-follow **#659** (this mission narrows FR-008/SC-5 accordingly to avoid breaking that active subsystem).
