...
- **Decision**: No new `__init__.py` exports. The shared helper is imported via `from doc_audit.judgment._llm_response import _strip_code_fence` (matching the existing import style in the package — the conftest at `tests/doc_audit/conftest.py` puts `scripts/` on `sys.path`, so `doc_audit.X` is the canonical package root used throughout `scripts/doc_audit/`). **Rationale**: Avoids changing package public surface (C-002).
