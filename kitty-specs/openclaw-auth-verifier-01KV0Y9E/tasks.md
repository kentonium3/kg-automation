...
- [ ] T001 Build `tests/security/fixtures/` healthy / shadow / drift fixtures
- [ ] T002 `findings.py` — `Finding` dataclass + sanitization invariant
- [ ] T003 `core.py` — discovery, sqlite read, AgentAuthState / PlaintextFileState, sha256, drift compare
- [ ] T004 `core.py` — Anthropic ping with timeouts and error classification
- [ ] T005 `anthropic-verify.sh` — bash entry with argparse, `--check` dispatch, lazy `--repair` import
- [ ] T006 `test_anthropic_verify_core.py` — topology + drift + mocked-ping tests
- [ ] T007 `test_anthropic_verify_output.py` — sentinel-grep + fs-snapshot tests
...
- [ ] T008 `repair.py` — backup + clear shadow rows + atomic plaintext rewrite
- [ ] T009 Wire repair dispatch in `__init__.py` (lazy import; FR-009 hint print)
- [ ] T010 `test_anthropic_verify_repair.py` — backup invariants + mutation tests
...
- [ ] T011 `anthropic-rotate.sh` — Step 6 verify gate + per-rotation manifest at `~/.cache/anthropic-rotate/manifest.<ts>.json`
- [ ] T012 `anthropic-rotate.sh` — `--rollback <ts>` argparse + restore logic
- [ ] T013 `test_anthropic_rotate_gate.py` — gate + rollback tests (bash-driven; mock the verifier output)
