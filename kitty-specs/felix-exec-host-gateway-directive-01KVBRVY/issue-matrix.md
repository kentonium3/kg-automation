...
|-------|-------|---------|--------------|
| #603 | Bug: inbox-5pm tool-invocation failure — `exec host=node requires a paired node` false-positive cron alert | in-mission | WP01 (commit 671fde82) adds the identical `host=gateway`-only hard rule to all four Felix sub-agent AGENTS.md; reaches terminal `fixed` after deploy + the 7-day zero-`host=node`-error window (NFR-002 / SC-003). |
