...
- **Risks**: verify-before-enable uses a **non-delivering `--self-test`** (NOT a real unit run — a real run would send an unscheduled report); transactional cutover guarantees exactly one producer; systemd-unit changes auto-rebaseline (repo-file signal); the openclaw-cron removal needs `expected_baselines`; escalation-daily stays an openclaw-cron (no service-inventory change — the fix just makes its runs succeed).

## Post-Plan Codex Review

A mandatory post-plan Codex review (spec-kitty-review profile) ran against spec.md + plan.md + research.md + data-model.md + contracts before task generation. It surfaced 12 findings (3 Critical, 6 High, 3 Medium), **all resolved or scoped**. The authoritative amendments are in **`contracts/post-plan-review-resolutions.md`** — that document governs where it conflicts with earlier artifacts (delivery-confirmation predicate, `--self-test` deploy gate, transactional cutover, pre-candidate/derive_state gating, due-date parsing, failure propagation, contracted systemd unit fields, tick schema). Two findings (label fetch strategy + label-form exclusion) are deliberately scoped to #716 (note posted). The 4-IC decomposition is unchanged; the contracts are tightened.
