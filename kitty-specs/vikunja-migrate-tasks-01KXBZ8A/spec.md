...
| FR-007 | Drop the deleted Goals project id (11) from **both** hardcoded exclusion seams — `scripts/common/vikunja_scope.py` (`ESCALATION_EXCLUDED_PROJECT_IDS` → `[13]`) and `scripts/enrichment/reconcile_completions.py` (`EXCLUDED_PROJECT_IDS` → `{13}`) — updating their tests. Habits(13) stays excluded; habit identity stays project-id based (Habits retained) with `t:habit` applied additively. Former Goals tasks (#1, #13) move to Intentional LLC and are **accepted as normal escalation candidates** (far-future due dates; no near-term change) — encoded by the updated scope test. The deployed `felix-admin-escalation` agent prompt (`TOOLS.md`, an audited surface) also names project 11 but is a harmless no-op after deletion and is **deferred to a follow-up** (avoids a deploy+rebaseline in this mission). | Accepted |
