...
| IC-07 | **Scope config** — drop dead Goals(11) from BOTH seams | `scripts/common/vikunja_scope.py`, `scripts/enrichment/reconcile_completions.py` | Remove `11` from `ESCALATION_EXCLUDED_PROJECT_IDS` → `[13]` and `EXCLUDED_PROJECT_IDS` → `{13}` (Habits 13 stays); leave `HABIT_SELECTOR` on project_id:13 (C-004). Update `tests/common/test_vikunja_scope.py` (assert `[13]`) and `tests/enrichment/test_reconcile_completions.py` (retarget the excluded-project fixture 11→13). Deployed `felix-admin-escalation` TOOLS.md prompt deferred (audited surface). FR-007, SC-006. |
