...
| FR-007 | The fail-safe MUST be preserved: any failure in step 1 (context-load) or step 2 (decision) MUST still result in `ESCALATE_TO_SONNET` with `fallback_invoked=true`. | Draft |
| FR-008 | The heartbeat-gate wiring MUST drop the now-unused LLM arguments and the Anthropic API-key dependency from the tick path (e.g. `--prompt` / `--api-key` / model args removed from the systemd `ExecStart` and `run.py`), without removing them from any path still legitimately needed. | Draft |
| FR-009 | The health-check MUST run via a **standalone non-agent execution path** (systemd timer or a non-agent openclaw cron) that executes the existing bash check and emits its result directly, **creating no Sonnet `main` session**. | Draft |
