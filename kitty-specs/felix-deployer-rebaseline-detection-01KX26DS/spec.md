...
| FR-004 | If the stored watermark SHA is unreachable/invalid (the range diff fails), felix-deployer MUST NOT crash: it logs the condition, treats the range as not determinable for that tick, and advances the watermark to the current HEAD so it self-heals on the next tick. | Draft |
...
|---|---|---|---|
| NFR-001 | The rebaseline path (observe, reconcile, watermark read/write) MUST NEVER crash the tick. | `run_tick` returns 0 under injected exceptions in every new code path; regression test asserts it. | Draft |
