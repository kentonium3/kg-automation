...
| IC-02 | Route `VikunjaClient` default-token loading through IC-01 (replace module `DEFAULT_TOKEN_PATH` literal). | FR-001 | Group-A consumers (bare `VikunjaClient()` — incl. `intake/scan_inbox.py`) inherit IC-01 automatically. |
...
| IC-06 | Docs/ADR: ADR-0007 (supersede ADR-0002 rationale); identity-model, credentials-and-secrets, service-inventory, data-flows; SKILL/TOOLS/AGENTS token refs + SKILL v2.4.0 + health-check fix (#831). **Reconcile obsolete in-code invariant comments**: `scan_inbox._build_client` "reads = felix-bot / never kent" docstring + sibling Group-A consumers + the `sync` systemd-unit path comment. | FR-006 | scan_inbox comment would contradict the flipped default if left. |
