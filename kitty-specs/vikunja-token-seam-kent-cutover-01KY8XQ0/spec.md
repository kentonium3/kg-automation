...
- The Phase-1 **Group-A** consumers (`route_and_finalize`, `route_someday`, `create_task`,
  `assertion_verifier`, `weekly_report_driver`, `query_active_habits_weekly`) already use bare
  `VikunjaClient()` — they inherit the flip automatically once the client default resolves through the helper.
