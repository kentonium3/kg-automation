...
| FR-004 | The spike MUST surface at least one genuine conflict or trade-off from the seeded chain that was reasoned (not hand-fed). | Draft |
| FR-005 | The spike MUST capture Kent's explicit usefulness judgment on Q2 — the make-or-break gate — after he has seen the FR-003 traversal and the FR-004 conflict. | Draft |
...
| FR-008 | The spike MUST record whether hand-seeding the slice reveals friction in the ontology tier model (Q4) — an empirical answer on a slice, not an exhaustive survey. | Draft |
...
| FR-010 | On a **go** verdict, the spike MUST confirm the phased rollout ladder (shadow → advisory → gated → autonomous) as the build/validation plan, sequence the epic (#693 → #694 → #695 → #696 → #697 → #698), and frame the membrane-topology decision for #696. | Draft |

### Non-Functional Requirements

| ID | Requirement | Measurable threshold | Status |
|---|---|---|---|
| NFR-001 | Zero production contact. | 0 writes/mutations to any production service, credential, or vault file during the spike. | Draft |
...
| NFR-004 | Footprint acceptability. | Measured peak memory/CPU/disk recorded as absolute numbers and judged against office2's remaining headroom; acceptability decision explicitly recorded. | Draft |
...
| ID | Criterion |
|---|---|
| SC-001 | A go/no-go verdict is produced with documented evidence for all four questions. |
| SC-002 | For Q2, Kent renders an explicit usefulness judgment after seeing at least one upward-traversal answer and at least one reasoned (not hand-fed) conflict/trade-off. |
