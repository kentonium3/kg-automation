...
| FR-005 | Kent shall supply missing fields in **compact shorthand**: one line per task keyed by the digest number. The line supplies **only the fields the digest reported missing** for that task (a task already carrying a valid `f:` or `q:` need not repeat it); the full form is `<n> <project> f<1-3> <quadrant>` plus optional Tier-2 tokens (a due date, `habit`, `loe:<s\|m\|l>`). Examples: `1 personal` (only project missing), `2 f2 schedule` (only labels missing), `3 clients f3 do due:fri`. A **deterministic parser** shall map the supplied tokens to the canonical project/labels and leave already-valid fields untouched. | Draft |
| FR-006 | Token resolution shall be **deterministic against the #748 seam / design-doc taxonomy**, case-insensitive, and shall accept the documented shorthands/aliases (e.g. `f2`→`f:2-growth`, `schedule`→`q:schedule`, a project short-name→its canonical project). The **LLM shall be invoked only** for a token that cannot be resolved deterministically, and its output shall be constrained to proposing a **canonical name** that the helper **re-resolves through the seam** — the LLM may never inject a raw label/project id or free-form value (Directive-6 boundary; FR-007 still enforces seam-only writes). | Draft |
...
| FR-008 | Applying a working project (≠ Inbox) + `f:` (1/2/3) + `q:` shall complete intake so the task **leaves Inbox** and no longer appears in subsequent scans. A `q:eliminate` assignment is also a resolution: the task is marked done (eliminated) rather than requiring a working project — it likewise stops appearing. These are the resolution mechanisms under the re-prompt-until-resolved policy (FR-011). | Draft |
| FR-009 | An `f:4-overload` assignment **is a resolution of the intake question** (Kent has decided the task needs breaking down): the apply attaches `f:4-overload`, records a **decomposition-pending** disposition, and emits one deterministic "needs decomposition" confirmation. A decomposition-pending task is **no longer counted as Tier-1-incomplete-for-prompting** (so it does not re-prompt every tick — resolving the infinite-loop under FR-011), and it is **never** routed to a working queue. Felix may surface decomposition-pending tasks in a separate, lower-frequency list rather than the intake digest. | Draft |
| FR-010 | **Applicable Tier-2** is evaluated at **apply time** (the scan cannot know the future quadrant): when the reply's quadrant is `q:do`/`q:schedule` and no `due:` was supplied, Felix emits a **non-blocking follow-up** asking for a due date; a supplied `due:` is written per the repository's ET end-of-day convention (avoiding the `T00:00:00Z` off-by-one class). `t:habit` is accepted when recurring; `loe:` when supplied. Tier-2 **never blocks** Tier-1 completion. Invalid Tier-2 combinations are handled by a deterministic compatibility matrix (FR-017), not silently applied. | Draft |
...
| FR-012 | Every reply line shall resolve to an explicit **per-line status**, echoed/confirmed to Kent, without blocking other lines: `applied`, `echoed_back` (unparseable/unresolvable — with what was understood and what failed), `overload_flagged` (FR-009), `noop` (FR-013), `not_found` (task id gone), `already_done`, `moved_conflict` (task left Inbox by another process), `access_denied` (kent-token cannot write). Nothing is silently applied or dropped. | Draft |
| FR-013 | Reply application shall be **idempotent, non-clobbering, and family-coherent**: writes use **read-modify-write** so unrelated labels and unstated fields are preserved (Vikunja POST `/tasks/<id>` zeros unstated fields). Because `q:` (and schedulable `f:`) are **mutually-exclusive families**, applying a new `q:`/`f:` **replaces** any existing label of that family (never leaving two quadrants) while preserving all non-family labels. A line is `noop` **only** when the task's live project/labels/due already match the intended values, or the task is done/deleted — a partially-resolved task still receives its still-missing/corrective fields. | Draft |
| FR-014 | The scan and apply shall emit a **deterministic observability signal** per tick covering scan counts (`scanned`, `incomplete`, `prompted`) **and** apply aggregates (`applied`, `echoed_back`, `overload_flagged`, `noop`, `not_found`, `already_done`, `moved_conflict`, `access_denied`, `failed`), sufficient for the health/doc-audit posture (observability-per-feature principle). | Draft |
| FR-015 | Documentation shall be synchronized: `docs/design/vikunja-configuration-design.md` (mark the validation loop implemented), `docs/design/architecture/data/` service-inventory + data-flow for the new intake scan + WhatsApp prompt flow (and md views), an intake-loop runbook, `docs/INDEX.md` **and `docs/DEVELOPER_PORTAL.md`** (per the signal-to-doc-map when a doc surface is added, #492) / roadmap as applicable, and #750 closure notes. | Draft |
| FR-016 | Reply→digest **correlation shall be collision-safe** across the 4 daily ticks: each digest is persisted as an **immutable per-`digest_id` record** (not an overwritten same-day file), with a `latest` pointer and a bounded retention window (48h, habits parity). The apply step shall select the correlated digest by the reply's **line-number set plus task-title/content evidence** (habits `correlate_reply_to_checkin` semantics) — never by position in the newest file alone — and shall `echoed_back` any line whose number has no unambiguous task. | Draft |
| FR-017 | A deterministic **Tier-2 compatibility matrix** shall govern Tier-2 tokens: `due:` with `q:eliminate` or with `f:4` → ignored-with-note (not applied); `habit` on a task with existing recurrence → note, no double-recurrence; malformed/out-of-set `loe:` → `echoed_back`. Each cell is accept-and-apply, ignore-with-note, or echo-back — never silent. | Draft |
...
- **SC-004:** A reply assigning `f:4-overload` records decomposition-pending, confirms once, does **not** schedule the task, and the task **stops re-prompting** in subsequent intake digests (no infinite loop).
- **SC-005:** Applying labels/project never removes a task's unrelated labels or zeros its other fields; a new `q:`/`f:` **replaces** the same-family label so a task never ends with two quadrants (read-modify-write + family-replace verified).
...
- **SC-007:** A `q:do`/`q:schedule` reply with no `due:` triggers a **non-blocking** due-date follow-up; a supplied due date is applied per the ET convention; Tier-2 absence never blocks Tier-1 completion; incompatible Tier-2 (`due:`+`q:eliminate`/`f:4`) is ignored-with-note.
...
- **SC-010:** A reply supplying only the missing field(s) (`1 personal`) applies correctly and leaves already-valid `f:`/`q:` untouched.
- **SC-011:** Across two same-day ticks with different digests, a delayed reply is correlated to the **correct** digest via line-number set + title evidence (no cross-digest number collision); an orphan number is echoed back.
- **SC-012:** A line whose task was deleted / already done / moved out of Inbox / write-denied returns the precise per-line status and does not block other lines.
