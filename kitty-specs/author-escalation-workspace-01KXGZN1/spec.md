...
| FR-010 | ~~The reschedule due-date examples in `TOOLS.md` and `AGENTS.md` are rewritten from `...T00:00:00Z` to the ET-offset form.~~ **REVERTED post-merge (2026-07-14).** The post-merge Codex review found the runtime reschedule helper (`scripts/escalation/record_completion.py:460`) also writes `...T00:00:00Z`, so the prompt-example fix created a prompt↔runtime contradiction and did not actually fix the off-by-one bug. Fixing the helper is a behavior change (violates NFR-004). The examples were reverted to `Z` to keep the prompt consistent with the current runtime; the real ET-vs-UTC bug is filed as **#733**. | Reverted → #733 |
