...
- **Rationale**: The #636 boundary: agent prompt files deploy via the `deploy_agent_prompts.py` pull pipeline, not felix-deployer manifests. Rebaseline is not required because `audited-surfaces.json` sets `rebaseline_required: false` for `openclaw-agent-prompts` (the #621 gap: `audit.sh` hashes only `openclaw.json`, not agent prompt files). Deploy dir is `/data/services/openclaw/tasker-agent/` (slug ≠ dir), to be re-verified via `find` at deploy time. tasker is a per-dispatch sub-agent (not the main DM lane), so no gateway restart is needed (C-007).
