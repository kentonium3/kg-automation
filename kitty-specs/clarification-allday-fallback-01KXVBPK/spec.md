...
| FR-005 | ONLY start-time-missing clarifications are eligible. Missing-title, unparseable-date, or any other/compound missing-field clarification MUST NOT be converted to an all-day event. | Approved |
| FR-006 | The all-day payload MUST be derived from the resolved date: `start_date` = resolved date, `end_date` = `start_date + 1 day` (exclusive end), with all timed fields (e.g. `start_rfc3339`, `start_timezone`) dropped. | Approved |
| FR-007 | The age-out create MUST be recorded in the routing log as a **distinct** event, separable from a normal timed/answered create and from a plain sweep-delete, so the operator can count appointments that landed via this fallback. | Approved |
| FR-008 | If the all-day create cannot complete, the operation MUST fail closed: the pending record is retained and the note is left unprocessed for a later sweep to retry; no partial or duplicate state is produced. | Approved |
...
| NFR-004 | Repeated sweeps over the same aged-out record (including after a transient failure) MUST converge to exactly one calendar event. | 1 event across N≥2 retries; 0 duplicates | Approved |
