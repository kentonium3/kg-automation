...
|----|-------------|--------|
| FR-001 | Every helper/script invocation in every active agent prompt (capture, escalation, habits, calendar, tasker, main; both `AGENTS.md` and any `AGENTS.md.tmpl`) MUST use the exec-sanitization-immune self-contained form anchored on the **exact** checkout path `/home/claude/kg-automation`: `cd /home/claude/kg-automation && python3 -m scripts.<pkg>.<mod>` and the analogous `cd /home/claude/kg-automation && python3 scripts/<path>.py`. This covers **all** invocation shapes currently present, including bare relative script paths (e.g. `invoke scripts/openclaw/agents/main/felix-file-issue.py`) and `python scripts/<path>.py` without a checkout-`cd`. No invocation may depend on an inherited `PYTHONPATH` or the deployed workspace cwd. | Approved |
| FR-002 | `env_assumptions.py` MUST be inverted to reflect the corrected canonical form: the self-contained checkout-`cd` (exact `/home/claude/kg-automation`) is compliant; the `${PYTHONPATH:?}` anchor is now a violation; **a relative `python3 -m scripts.…` OR `python3 scripts/<path>.py` OR a bare imperative `scripts/<path>.py` not preceded by the checkout-`cd` is a violation** (a new relative-script class parallels `BARE_M_SCRIPTS`). The Test-CI fleet guard, `validate_workspace.check_runtime_env_assumptions`, and all associated tests MUST agree with the new policy. Every runtime-reliable prompt passes the checker; no reliable prompt is flagged; no broken form passes. | Approved |
...
| C-006 | Docs MUST be updated in-mission: `data/service-inventory.json` (+ md) capture `model` haiku→sonnet **and** the openclaw-gateway PYTHONPATH-drop-in claim corrected (drop-in sets gateway-process env but exec strips it); `docs/constitution/AGENT-REGISTRY.md` **and** the authoritative `docs/constitution/agent-registry.json` (capture model); `docs/runbooks/openclaw-agent-setup.md` (it currently *teaches* the broken `${PYTHONPATH:?}` form and bans the now-canonical checkout-`cd` — this MUST be corrected so future authoring doesn't reintroduce the bug). | Approved |
...
- **SC-007**: The merge records `Rebaseline: completed at <ts>` (manual, for the openclaw.json model change); `service-inventory.json` (+ md), `AGENT-REGISTRY.md`, `agent-registry.json`, and `openclaw-agent-setup.md` all reflect the new model and the corrected canonical invocation form.
- **SC-008**: Test-CI green — the inverted `env_assumptions.py` (incl. the relative-script violation class) + updated tests pass; no reliable prompt is flagged and no broken form (bare/relative/`${PYTHONPATH:?}`) passes.
