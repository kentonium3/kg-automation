...
**Status**: Planning-time draft. The live version will be created at `docs/design/architecture/contracts/drift-ledger-schema.md` during implementation. This file in the mission's `contracts/` directory is the spec-kitty planning record per the standard mission layout.
