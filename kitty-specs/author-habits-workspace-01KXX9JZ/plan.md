...
**Testing**: habits-scoped `validate_workspace.py --json` assertion (`felix-admin-habits` → `ok:true`); row-by-row content-conservation check; before/after `scripts.habits.*` morning-list helper output diff; post-deploy live smoke
...
**Structure Decision**: No new source structure. This is markdown authoring within one existing agent directory plus reuse of the existing validator. The authoritative surface is `scripts/openclaw/agents/felix-admin-habits/`.
...
- **Purpose**: Re-home content to #587 ownership and prove the set is coherent, invariant-green, and behavior-preserving.
- **Relevant requirements**: FR-001…FR-009, FR-011, NFR-001, NFR-002, NFR-003, NFR-004
- **Affected surfaces**: `scripts/openclaw/agents/felix-admin-habits/{SOUL,USER,TOOLS,AGENTS}.md`; reuses `validate_workspace.py`
- **Sequencing/depends-on**: none (branch already carries #587 from main)
- **Risks**: stripping the enforceable privacy rule from all files (Inv-A regression); dropping the weekly-out-of-scope statement from both AGENTS and SOUL; de-inlining IDs that a runtime path actually depends on (validate helper resolves by name first)
