...
| FR-008 | `TOOLS.md` no longer inlines volatile Vikunja IDs: the `Habits` project `(id=13)` parenthetical and the `Habit task IDs: 14-20 (…)` line are removed; the completion-comment storage contract (one task per habit; idempotent daily completion comment; comment format) is retained. **Mechanism (corrected per post-plan review, Finding 1):** the inlined IDs are consumed by neither the deterministic tick/reply helpers nor the agent, so removing them is behavior-preserving — the helpers resolve the Habits project via `scripts/common/vikunja_refs.json` (`vikunja_scope`, `HABITS_PROJECT_ID`) and derive the active task set from the sync cache + `scripts/habits/migrations/phase3-schedule.yaml` + the persisted morning artifact, **not** from `TOOLS.md`. TOOLS instructs name-based resolution only for the agent's own ad-hoc habit-management operations via the `vikunja_api` skill (the LLM path), and points to `vikunja_refs.json` as the canonical id source instead of inlining ids. | Draft |
...
| FR-010 | The updated habits workspace files deploy to office2 via agent-prompt-sync on merge to `main` (no `deploys/queued/` manifest); repo ↔ office2 md5 parity is verified post-deploy at the confirmed destination directory `/data/services/openclaw/habits-agent/` (per `service-inventory.json`; agent slug ≠ deploy dir — confirmed via post-plan review Finding 5). | Draft |
| FR-011 | #409 (SOUL-vs-AGENTS weekly-report standing-orders conflict) is confirmed resolved and incorporated **within the workspace prompt files**: after FR-003 there is a single authoritative weekly-report-out-of-scope statement (in AGENTS), no in-workspace contradiction remains, and #409 stays closed with a pointer to this mission. (Repo-wide doc coherence for the weekly-report move is handled by FR-012.) | Draft |
| FR-012 | The stale weekly-report description in `docs/design/architecture/service-inventory.md` (narrative rows still describing a weekly OpenClaw cron via `felix-admin-habits`, contradicting `service-inventory.json` which correctly attributes weekly reporting to the deterministic `felix-habits-weekly` timer, #723) is corrected to match the JSON authority. Bounded to the weekly-report lines only. (Post-plan review Finding 4.) | Draft |
...
| NFR-002 | Scope discipline | The mission diff touches only: habits `SOUL.md`/`USER.md`/`TOOLS.md` (and `AGENTS.md` narrowly, FR-009 only, if warranted); the weekly-report lines of `docs/design/architecture/service-inventory.md` (FR-012, bounded); plus mission artifacts — no other agent, no IDENTITY.md, no unrelated file. | Draft |
...
| NFR-004 | Behavior preservation | Two-part (corrected per post-plan review, Finding 2): **(a) scope-creep guard** — before/after output of the morning-list helper for the same date is identical, proving the edit touched no helper/config state (a prompt-only change cannot alter deterministic helper output, so this alone is NOT a prompt-behavior gate); **(b) prompt-behavior guard** — static-diff assertions that the AGENTS tick/reply workflow commands, relay-verbatim rule, Output Discipline block, completion-marking flow, and habit-management rules are unchanged, PLUS the post-deploy live smoke as the actual prompt-mediated behavior check. | Draft |
...
6. #409 is confirmed resolved/incorporated in the workspace (single coherent weekly-report statement; no conflict remains).
7. `service-inventory.md` weekly-report rows are corrected to match `service-inventory.json` (FR-012) — repo-wide weekly-report doc coherence.
...
- The deterministic tick/reply helpers (`scripts.habits.*`) do **not** read `TOOLS.md`: the Habits project resolves via `scripts/common/vikunja_refs.json` (`vikunja_scope`) and the active task set from the sync cache + `scripts/habits/migrations/phase3-schedule.yaml` + the persisted morning artifact (confirmed in the post-plan review — corrects the earlier "resolve by name" phrasing). Removing the inlined IDs from `TOOLS.md` (a prompt/doc file that is authority for neither the helpers nor the agent) is therefore behavior-preserving. Name-based resolution via the `vikunja_api` skill applies only to the agent's ad-hoc habit-management path.
