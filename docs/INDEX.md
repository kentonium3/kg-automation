...
- [Spec-Kitty Upstream Filing Packet — rc4](<./diagnostics/spec-kitty-upstream-filing-packet-rc4.md>) — work order for the spec-kitty QA bot: five ready-to-post upstream issue filings (one new defect, three persistence-after-closure, one verification report) with suggested labels, plus what is verified fixed and the do-not-file list; exists because upstream label/priority privileges sit with the work-hat account, not the personal hat that does the analysis
