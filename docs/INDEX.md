...
- [Spec-Kitty Upstream Filing Packet — rc4](<./diagnostics/spec-kitty-upstream-filing-packet-rc4.md>) — work order for the spec-kitty QA bot: three ready-to-post upstream issue filings with suggested labels, plus the do-not-file dedupe list; exists because upstream label/priority privileges sit with the work-hat account, not the personal hat that does the analysis
