...
- [Spec-Kitty 4.0.0rc4 Upgrade Findings](<./diagnostics/spec-kitty-4.0.0rc4-upgrade-findings.md>) — QA-agent-facing register of the 15 findings from the 3.2.7 → 4.0.0rc4 → rc5-dev upgrade across five builds (2026-09-17→23); build-pinned, maps each finding to its kg-automation issue and upstream fix commit
- [Spec-Kitty Upstream Filing Packet — baseline anomalies](<./diagnostics/spec-kitty-upstream-filing-packet-anomalies.md>) — work order for the spec-kitty QA bot: two approved new issues (P0: CRLF skill sources make the installer double every SKILL.md frontmatter and --fix cannot converge; P3: dry-run PAYLOAD_INVALID rows render blank and 9.1.6 transition rules reject the CLI's own 2026-03 history) with filing mechanics and a do-not-file ledger (2026-09-23)
- [Spec-Kitty Upstream Filing Packet — PR #4947](<./diagnostics/spec-kitty-upstream-filing-packet-pr4947.md>) — work order for the spec-kitty QA bot: two approved comments (PR #4947: #4923 confirmed fixed, #4927 not fixed with plan-JSON root cause; #4925: the requested Windows re-verify plus the exit-code trace), a do-not-file ledger, and post-filing steps (2026-09-23)
- [Spec-Kitty Upstream Filing Packet — rc4](<./diagnostics/spec-kitty-upstream-filing-packet-rc4.md>) — work order for the spec-kitty QA bot: six ready-to-post upstream posts (one new defect, three persistence-after-closure, one verification report, one comment on open #4902) with suggested labels, plus what is verified fixed and the do-not-file list; exists because upstream label/priority privileges sit with the work-hat account, not the personal hat that does the analysis
