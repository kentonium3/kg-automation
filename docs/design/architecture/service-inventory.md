...
- **Model**: `anthropic/claude-sonnet-4-6` (as of 2026-07-15, #743). Previously `anthropic/claude-haiku-4-5`. The haiku evaluation concluded against haiku: it was marking inbox notes `processed` **without routing their content while reporting success** — confirmed silent loss of two notes (2026-07-13 items, and 2026-07-14). #662's earlier reliability fix was environmental (OpenClaw's exec tool strips PYTHONPATH; corrected fleet-wide by self-contained `cd /home/claude/kg-automation && python3 -m scripts.…` invocations), but the residual failures were instruction-following / tool-call-discipline gaps that a stronger model buys down. Swap is a frequency-reducer; the structural fix (extend #737's atomic finalize to all routes so "processed-but-empty" is unreachable) is tracked separately. See #671 for the fleetwide model-selection framework this feeds.
