...
| Habit Report (weekly) | Sunday 10 PM ET (cron `0 22 * * 0` America/New_York) | OpenClaw cron → felix-admin-habits → `scripts/habits/query_active_habits_weekly.py` → `scripts/common/vikunja_client.py` → Vikunja API (done_at history) | claude | Weekly habit pattern report via WhatsApp — deterministic-helper-backed post mission `vikunja-client-and-habits-weekly-report-01KTKSFT` (#542 + #562). Pre-mission, the agent improvised data via LLM reasoning, producing confabulated percentages. |
...
- **Purpose**: Daily habit check-in delivery, completion tracking, weekly pattern reports, on-demand track record queries, habit management (add/pause/remove). **Post-#371**: thin orchestrator for the morning + reply ticks. The morning tick invokes `scripts/habits/morning_checkin_list.py` (script writes the canonical morning-list artifact + emits the formatted WhatsApp message verbatim); the reply tick invokes `scripts/habits/parse_morning_reply.py` (deterministic mapping of Kent's reply against the persisted morning list) and routes the resulting `(task_id, state)` tuples to the existing `scripts/habits/record_completion.py`. The narrow LLM judgment surface `scripts/habits/judgment/disambiguate_reply.py` is invoked ONLY for ambiguous reply tokens (mirrors the #343 doc-audit judgment pattern). **Post mission `vikunja-client-and-habits-weekly-report-01KTKSFT`**: the weekly Sunday-22:00 tick is also a thin orchestrator — the agent invokes `scripts/habits/query_active_habits_weekly.py` which queries Vikunja API directly via the new shared `scripts/common/vikunja_client.py` for `done_at` history, rolls up by canonical habit title, and emits a `WeeklyHabitReport` JSON the agent renders to WhatsApp. The weekly path goes direct (NOT through the sync cache) because the sync cache holds current state only and lacks `done_at` history. Pre-mission behavior was LLM-improvised (confabulated percentages, uniform-zero prior-week baselines); the deterministic helper is the canonical replacement.
- **Schedule**: Morning check-in at 7:05 AM ET daily, weekly report Sunday 10 PM ET (cron `0 22 * * 0` America/New_York, verified phase-0 of mission `vikunja-client-and-habits-weekly-report-01KTKSFT`)
...
- **scripts/habits/query_active_habits_weekly.py** (script, introduced_by mission `vikunja-client-and-habits-weekly-report-01KTKSFT`) — Sole source of the weekly Sunday-22:00 habit-pattern report. Instantiates the shared `scripts/common/vikunja_client.VikunjaClient` and issues `done_at`-bounded queries (current 7-day window + prior 7-day baseline) against project 13 (`Habits`). Rolls up by canonical habit title — daily-cadence habits (`repeat_after == 86400`) and weekday-in-title habits (e.g., `Strength training — Monday`) are classified per the `HabitClassifier` rules and counted against per-kind scheduled-days math (7 for daily, 1 per matched-weekday occurrence for weekday-in-title). Non-habit project-13 tasks (e.g., one-off cardiac-task class with `repeat_after == 0` and no weekday in title) are filtered at the helper layer per FR-006. Emits a `WeeklyHabitReport` JSON on stdout per the spec's Key Entities shape; the agent renders verbatim. Standard library only beyond the shared client. Failure mode: typed `VikunjaError` exceptions propagate; agent emits `Weekly report unavailable: <reason>` rather than fabricating data.
  - **runs_on**: `office2`
  - **invoked_by**: `felix-admin-habits` agent (cron `habits-weekly-report`, `0 22 * * 0` America/New_York)
