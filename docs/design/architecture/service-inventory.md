...
- **Model**: `anthropic/claude-sonnet-4-6` — upgraded from `anthropic/claude-haiku-4-5` by `harden-inbox-capture-01KWVGZM` (#662, 2026-07-06). Haiku persistently misread the helper-invocation prose as "missing infrastructure" and hallucinated failures after the #656/#658 env changes; Sonnet's stronger prompt comprehension is the fix, co-shipped with self-contained `cd /home/claude/kg-automation && …` helper invocations (see #658). (This narrative previously read haiku while `data/service-inventory.json` already read sonnet — the split-brain is now reconciled to sonnet.)
