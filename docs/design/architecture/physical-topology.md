...
- **Tailscale SSH is enabled on office2 and office4** (`tailscale up --ssh`; confirmed via `tailscale debug prefs` showing `RunSSH: true` on both hosts — office4 since 2026-08-29, #932, so the two hosts use one mechanism). tailscaled intercepts incoming SSH on port 22 of the Tailscale IP and, with `action: "accept"`, **terminates** the connection and authorises it on tailnet device identity. It does **not** pass through to sshd, and `authorized_keys` is never consulted on this path — which is why `PermitRootLogin no` in `sshd_config` did not prevent root logins until `root` was removed from the ACL's `users` list (#931, #932). Current ACL: `action: "accept"`, `src: autogroup:member` → `dst: autogroup:self`, `users: ["kgale", "claude", "codex"]`. The change from `check` to `accept` is documented in [ADR-0004](<./adr/0004-tailscale-ssh-with-accept-acl.md>). The LAN path (192.168.1.158 for office2) bypasses Tailscale SSH entirely and still enforces `authorized_keys`; see [security-posture § SSH access](<./security-posture.md>).
  - **Corrected 2026-09-11 (#972).** This bullet previously carried three stale claims: that Tailscale SSH was office2-only with office4 at `"RunSSH": false`; that the `accept` action passes the connection through to sshd; and the pre-#932 users list `autogroup:nonroot, root`. `data/network-topology.json` is authoritative on all three.
