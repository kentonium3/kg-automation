...
### Follow-on Implementation Missions (will be filed under Epic #507)

Three follow-on missions are scoped from the recommendation, in dependency order:

1. **Mission A — Sync reconciliation driver foundation.** Build the central reconciliation driver (scripts-first Python + systemd user timer) that runs the 6-phase cycle for the status + task layers. Implements the conflict-event log JSONL and the WhatsApp-router integration for unsafe-class conflicts. Does NOT include project-layer reconciliation or migration of existing touchpoints. No predecessors.

2. **Mission B — Touchpoint migration to sync cache.** Migrate the existing habits/escalation/tasker touchpoints (TP-01..TP-15E, TP-16A-E, TP-18) from direct Vikunja API calls to consulting the sync cache where appropriate. Removes the duplicate-poll pattern (each script polling Vikunja independently). Depends on Mission A.

3. **Mission C — Project layer + deletion handling + URL normalization.** Adds project-layer polling, the 3-cycle deletion-confirmation algorithm for use case (b), and resolves the two-URL-base fragility surfaced in RQ-2 (Tailscale HTTPS vs direct IP HTTP). Depends on Mission A.

The exact GitHub issue numbers will be back-filled in this section once the sub-issues are filed.
