...
- It returns ONE JSON object on stdout with a `status`; branch on it (AGENTS.md → Step 3):
  - `{"status":"created","mode":"conversational"|"clarification","event_id":…,"html_link":…,"summary":…,"start":…}` — `start` is the calendar's ACTUAL ET date/time (report it as-is). A clarification result also carries `"cleanup_ok": <bool>` (and a `cleanup` block): the orchestrator best-effort removes the pending record, flips the source note, and logs — so `cleanup_ok: false` means the event exists but the reminder wasn't cleared (surface that to Kent). On an idempotent hit (a re-reply matched an EXISTING event) it also carries `"idempotent": true`; if your requested time differed it adds `"time_change_applied": false`, `"requested_start":…`, and a `"note"` — surface the `note` verbatim (the calendar was NOT moved) (#838).
