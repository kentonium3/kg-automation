...
1. **Record the pending clarification WITH its eligibility signal.** Build the `--partial-payload` JSON from the calendar validation output (`validate_calendar_event`'s `complete: false` result — WP01/#780). It MUST carry the `title`, the resolved `start_date` (`YYYY-MM-DD`), and the `missing_fields` list (e.g. `["start_time", "end_or_duration"]`) — minimum shape `{"title": "<title>", "start_date": "<YYYY-MM-DD>", "missing_fields": [...]}`. **Copy `start_date` and `missing_fields` byte-for-byte from that validation output — do NOT compute, guess, or reformat the date, and do NOT re-derive the missing list yourself.** Determinism lives in code, not in this prompt; your only job is to carry the fields through verbatim. Then run `cd /home/claude/kg-automation && python3 -m scripts.inbox.handle_clarification_state add --note-filename <name> --partial-payload <json>` (the JSON-array store at `/data/services/openclaw/state/pending-calendar-clarifications.json`). This recorded signal is exactly what the 8h sweep-finalize (Step 1a) reads to decide all-day-fallback eligibility.
