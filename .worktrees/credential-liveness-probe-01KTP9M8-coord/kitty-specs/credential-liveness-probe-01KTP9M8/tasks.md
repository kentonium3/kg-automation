...
- [x] T001 Add `LivenessProbeConfig` dataclass to `manifest.py` (WP01)
- [x] T002 Extend `Credential` dataclass with `liveness_probe: Optional[LivenessProbeConfig] = None` (WP01)
- [x] T003 Update manifest parser: read the block, validate required-when-enabled, raise `ManifestQualityError` on unknown subkeys (WP01)
- [x] T004 Update `docs/design/architecture/data/credential-manifest.json` `gog-credentials-keyring` record with the new block (WP01)
- [x] T005 Add tests in `test_manifest.py` for: parse-with-block, parse-without-block, parse-disabled, missing-required-when-enabled, unknown-subkey (WP01)
...
- [x] T006 Create `liveness.py` skeleton with `LivenessClassification` Literal + `LivenessResult` dataclass (frozen=True) (WP02)
- [x] T007 Implement `probe_oauth_liveness()` happy-path branch (subprocess call → exit 0 → return None + log credential_alive) (WP02)
- [x] T008 Implement `invalid_grant` detection + classification window (keyring mtime + 7d ± 24h → routine-7day vs unexpected) (WP02)
- [x] T009 Implement probe-error branches (TimeoutExpired, FileNotFoundError, non-zero exit without invalid_grant) (WP02)
- [x] T010 Add structured INFO logging (credential_alive, credential_dead, credential_probe_error with all required fields) (WP02)
- [x] T011 Create `tests/security/credential_health_check/test_liveness.py` with 13 contract test cases (alive, routine, unexpected-early, unexpected-late, boundaries, timeout, missing binary, other failure, recovery_command in dead, recovery_command None in error, raises if disabled, UTC probed_at) (WP02)
- [x] T012 Verify coverage gate locally: `pytest --cov-branch --cov-fail-under=90` passes (WP02)
...
- [x] T013 Add `_process_liveness_alert()` to `orchestrator.py` per the contract (probe → dedup → file_alert flow) (WP03)
- [x] T014 Modify `run_cycle()`: add `liveness_only` kwarg; invoke `_process_liveness_alert` in both modes (defense-in-depth per plan §IC-03) (WP03)
- [x] T015 Add `liveness_alerts_filed: int = 0` field to `CycleResult` (additive) (WP03)
- [x] T016 Add `--liveness-only` flag to `__main__.py`; plumb to `run_cycle()` (WP03)
- [x] T017 Extend `--list --liveness` in `listing.py` per the scope-reduced contract (5 columns: name, enabled, gog_account, keyring_mtime_age, expected_next_expiration, recovery_command — NO live classification column) (WP03)
- [x] T018 Add tests in `test_orchestrator.py` + `test_listing.py` covering: skip-if-no-block, file-on-routine, separate-issue-on-unexpected, dedup-routine, dry-run-no-file, probe-error-no-file, liveness-only-skips-others, run_cycle-in-both-modes, listing-table-shape (WP03)
...
- [x] T019 Create `scripts/office2/credential-liveness-probe.service` per contract (oneshot; HOME/PYTHONPATH/EnvironmentFile mirror existing pattern) (WP04)
- [x] T020 Create `scripts/office2/credential-liveness-probe.timer` per contract (`OnCalendar=*-*-* 00,06,12,18:00:00`, Persistent=true) (WP04)
- [x] T021 Create `scripts/office2/deploy/credential-liveness-probe.sh` per contract (idempotent; checks `liveness.py` and `gog` binary presence; git pull; cp units; daemon-reload; enable --now; smoke-test dry-run) (WP04)
- [x] T022 Make deploy script executable: `chmod 755 scripts/office2/deploy/credential-liveness-probe.sh` (WP04)
