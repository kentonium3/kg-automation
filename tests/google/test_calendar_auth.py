"""Unit tests for ``scripts.google.calendar_auth`` (mission WP01).

CI-safe by construction: the real ``google-*`` packages are NOT installed here
or in CI (they live only in the office2 venv, research D3). These tests inject
fakes into ``sys.modules`` for ``google``, ``google.oauth2.credentials``,
``google.auth.transport.requests``, and ``googleapiclient.discovery`` **before**
importing the module under test, so nothing requires the real libraries and no
test touches the network. The module's google imports are lazy (inside
``load_credentials``), so the fakes are resolved at call time.

Canonical invocation (repo threshold 90):

    pytest tests/google/test_calendar_auth.py \
        --cov=scripts.google.calendar_auth --cov-branch --cov-fail-under=90

Covers:
- valid token → returned unchanged;
- expired-but-refreshable → ``refresh`` called + persisted;
- ``invalid_grant`` / ``RefreshError`` → ``CalendarAuthError`` (no interactive
  flow attempted);
...
class _FakeCredentials:
    ...
    ``expired`` / ``refresh_token`` surface that ``load_credentials`` inspects.
    ``from_authorized_user_file`` is monkeypatched per-test to return a
    configured instance (or raise), so the file's actual contents are irrelevant.
    """
...
def _install_fake_google(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    discovery_mod.build = lambda *a, **k: None  # type: ignore[attr-defined]
...
@pytest.fixture()
def cal_auth(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    """Import ``scripts.google.calendar_auth`` with fake google libs installed.
    ...
    module = importlib.import_module("scripts.google.calendar_auth")
...
def test_scopes_default_is_calendar_events(cal_auth):
    assert cal_auth.SCOPES_DEFAULT == [
        "https://www.googleapis.com/auth/calendar.events"
    ]


def test_credential_dir_honors_felix_google_dir(cal_auth, tmp_path):
    assert cal_auth.credential_dir("personal") == tmp_path / "personal"
    assert cal_auth.credential_dir() == tmp_path / "personal"  # default account


def test_credential_dir_custom_account(cal_auth, tmp_path):
    assert cal_auth.credential_dir("intentional") == tmp_path / "intentional"


def test_client_secret_and_token_paths(cal_auth, tmp_path):
    assert cal_auth.client_secret_path("personal") == (
        tmp_path / "personal" / "client_secret.json"
    )
    assert cal_auth.token_path("personal") == tmp_path / "personal" / "token.json"
...
def test_bad_account_name_raises_value_error(cal_auth, bad):
    with pytest.raises(ValueError):
        cal_auth.credential_dir(bad)


@pytest.mark.parametrize("good", ["personal", "intentional", "a", "a1_b-2", "0start"])
def test_good_account_names_accepted(cal_auth, tmp_path, good):
    assert cal_auth.credential_dir(good) == tmp_path / good


def test_load_credentials_rejects_bad_account(cal_auth):
    with pytest.raises(ValueError):
        cal_auth.load_credentials("../evil")
...
def test_valid_token_returned_unchanged(cal_auth, monkeypatch):
    creds = _FakeCredentials(valid=True, expired=False)
    monkeypatch.setattr(
        cal_auth,
        "token_path",
        lambda account="personal": cal_auth.credential_dir(account) / "token.json",
    )
    ...
    result = cal_auth.load_credentials("personal")
...
def test_expired_refreshable_token_is_refreshed_and_persisted(cal_auth, monkeypatch):
    ...
    assert cal_auth.token_path("personal").exists()


def test_refresh_error_raises_calendar_auth_error_no_interactive(cal_auth, monkeypatch):
    ...
    with pytest.raises(cal_auth.CalendarAuthError) as excinfo:
        cal_auth.load_credentials("personal")

    assert creds.refresh_called is True  # refresh attempted, then failed
    assert "re-mint token on the Mac" in str(excinfo.value)


def test_missing_token_raises_calendar_auth_error_with_remint(cal_auth):
...
def test_expired_without_refresh_token_fails_safe(cal_auth, monkeypatch):
...
def test_unreadable_token_file_raises_calendar_auth_error(cal_auth, monkeypatch):
    _seed_token(cal_auth)
...
def test_custom_scopes_appear_in_remint_message(cal_auth):
    with pytest.raises(cal_auth.CalendarAuthError) as excinfo:
        cal_auth.load_credentials(
            "personal", scopes=["https://www.googleapis.com/auth/calendar"]
        )
    assert "https://www.googleapis.com/auth/calendar" in str(excinfo.value)


# --------------------------------------------------------------------------- #
# Persistence perms (T002/T003)
# --------------------------------------------------------------------------- #


def test_persisted_token_perms_are_0600_and_dir_0700(cal_auth, monkeypatch):
    creds = _FakeCredentials(valid=False, expired=True, refresh_token="rt-xyz")
    _seed_token(cal_auth)
    monkeypatch.setattr(
        sys.modules["google.oauth2.credentials"].Credentials,
    ...
    tok = cal_auth.token_path("personal")
    cred_dir = cal_auth.credential_dir("personal")
...
def test_to_json_contents_never_returned_as_string(cal_auth, monkeypatch):
